  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      void main() {
        ...
        // WebGL 1.0 loop must use constant-bound integer expression
        for(int i = 0; i < 4; i++) {
          uv = fract(uv * 1.5) - 0.5;
          
          d = length(uv) * exp(-length(uv0));
          vec3 color = palette(length(uv0) + float(i) * 0.4 + u_time * 0.01);
          
          d = sin(d * 4.0 + u_time) / 36.0;
          d = abs(d); // Prevent division by zero / negative values for pow()
    ...
    // Dynamic resize handler: reads canvas display client size directly inside render loop
